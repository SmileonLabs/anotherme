...
AI가 판정하는 말발 배틀 앱 — 1:1/그룹 채팅, 성장RPG, 토크배틀, 음성통화를 갖춘 한국어 소셜 앱.
...
- `artifacts/landing/` — react-vite marketing/landing page (slug `landing`, served at `/landing/`). 3 CTAs: PDF download (`public/anotherme.pdf`), Android APK (`src/config.ts` → `APK_URL`, points to the direct EAS `.apk` artifact URL — re-swap after each new EAS build), PWA install (`beforeinstallprompt` + iOS manual-install fallback modal). Installable PWA: `public/manifest.webmanifest` (relative `start_url`/`scope` so it's base-path-portable), `public/sw.js`, `public/icon-{192,512}.png`. All public-asset refs use `import.meta.env.BASE_URL` — never hardcode `/landing/`.
...
- **성장RPG** — AI 성장RPG 마스터가 진행하는 그룹 텍스트 어드벤처
