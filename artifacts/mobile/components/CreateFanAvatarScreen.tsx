...
export default function CreateFanAvatarScreen() {
  ...
  const { fanProfiles, createFanProfile, isCreatingFan } =
  ...
  const recipe = buildAvatarRecipe("fan", [
    "fan.background.001",
    baseKey,
    headKey,
    wearKey,
    ...
    try {
      ...
      const locked =
        ...
        (error instanceof Error &&
          error.message.includes("FAN_EXPANSION_LOCKED"));
      crossAlert(
        "FAN 생성 안내",
        locked
          ? "기존 FAN이 토르미아 또는 Lv.50에 도달하면 새 FAN을 만들 수 있어요."
          : "FAN 캐릭터를 만들지 못했어요. 핸들과 입력값을 확인해 주세요.",
      );
    }
  };
  ...
  return (
    <NeonBackdrop style={styles.root}>
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.title}>새 FAN 만들기</Text>
        <View style={{ width: 28 }} />
      </View>
      <ScrollView
      ...
      >
        <Text style={styles.lead}>
          {canCreate
            ? "캐릭터의 모습을 선택하세요. 생성 후에도 아바타 꾸미기에서 변경할 수 있어요."
            : "이름과 모습을 먼저 정할 수 있어요. FAN 추가는 기존 FAN이 토르미아 또는 Lv.50에 도달하면 열립니다."}
        </Text>
        ...
        <TextInput
          ...
          style={styles.input}
        />
        <Text style={styles.label}>공개 핸들 (선택)</Text>
        <TextInput
          editable={!isCreatingFan}
          value={handle}
            ...
            {(["woman", "man"] as const).map((value) => (
              ...
              >
                <Text
                  style={[
                    styles.genderText,
                    gender === value && styles.choiceTextActive,
                  ]}
                >
                  {value === "woman" ? "여성" : "남성"}
                </Text>
        ...
        <ChoiceRow
          label="헤어"
          count={gender === "man" ? 6 : 5}
          selected={head}
          onSelect={setHead}
        />
        <ChoiceRow label="의상" count={6} selected={wear} onSelect={setWear} />
        <Pressable
          disabled={!canCreate || !displayName.trim() || isCreatingFan}
          onPress={() => void submit()}
          style={[
            styles.submit,
            (!canCreate || !displayName.trim() || isCreatingFan) &&
              styles.disabled,
          ]}
        >
          {isCreatingFan ? (
            ...
            <>
              <Feather
                name={canCreate ? "star" : "lock"}
                size={18}
                color="#fff"
              />
              <Text style={styles.submitText}>
                {canCreate ? "FAN 소환하기" : "추가 조건 미달"}
              </Text>
            </>
          )}
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  lead: { color: "#BEB7CC", fontSize: 14, lineHeight: 21, marginBottom: 2 },
  preview: {
    alignSelf: "center",
    width: 245,
    height: 350,
  ...
  label: { color: "#F5F1FF", fontSize: 13, fontWeight: "700" },
  input: {
    minHeight: 48,
    borderWidth: 1,
    borderColor: "#49336A",
  ...
  choiceActive: { backgroundColor: "#743BF5", borderColor: "#B778FF" },
  choiceText: { color: "#A9A1B8", fontSize: 13, fontWeight: "700" },
  ...
  genderButtonActive: { backgroundColor: "#743BF5", borderColor: "#B778FF" },
  genderText: { color: "#A9A1B8", fontWeight: "800" },
  submit: {
    marginTop: 18,
    minHeight: 54,
    borderRadius: 18,
    backgroundColor: "#743BF5",
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 8,
