...
export default function CreateFanProfileScreen() {
  ...
  const { createFanProfile, isCreatingFan } = useCharacterProfiles();
  ...
  const submit = async () => {
    ...
    try {
      await createFanProfile({ displayName: displayName.trim(), ...(handle.trim() ? { handle: handle.trim() } : {}), customization: selection });
      router.replace("/(tabs)/persona" as never);
    } catch (error) {
      const locked = error instanceof Error && error.message.includes("FAN_EXPANSION_LOCKED");
      crossAlert("FAN 생성 안내", locked ? "기존 FAN이 토르미아 또는 Lv.50에 도달하면 새 FAN을 만들 수 있어요." : "FAN 캐릭터를 만들지 못했어요. 핸들과 입력값을 확인해 주세요.");
    }
  };
  return <NeonBackdrop style={styles.root}><View style={[styles.header, { paddingTop: insets.top + 8 }]}><Pressable onPress={() => router.back()}><Feather name="chevron-left" size={28} color="#fff" /></Pressable><Text style={styles.title}>새 FAN 만들기</Text><View style={{ width: 28 }} /></View><ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 32 }]}><Text style={styles.lead}>독립된 피드와 성장을 가진 FAN 캐릭터를 만드세요.</Text><Text style={styles.label}>캐릭터 이름</Text><TextInput value={displayName} onChangeText={setDisplayName} maxLength={30} placeholder="이름 입력" placeholderTextColor="#777184" style={styles.input} /><Text style={styles.label}>공개 핸들 (선택)</Text><TextInput value={handle} onChangeText={setHandle} autoCapitalize="none" maxLength={24} placeholder="fan-name" placeholderTextColor="#777184" style={styles.input} />{(Object.keys(OPTIONS) as Array<keyof typeof OPTIONS>).map((key) => <View key={key} style={styles.section}><Text style={styles.label}>{key === "ageStyle" ? "연령 스타일" : key === "hairStyle" ? "헤어" : key === "skinTone" ? "톤" : "표현"}</Text><View style={styles.options}>{OPTIONS[key].map((value) => <Pressable key={value} onPress={() => setSelection((current) => ({ ...current, [key]: value }))} style={[styles.chip, selection[key] === value && styles.chipActive]}><Text style={[styles.chipText, selection[key] === value && styles.chipTextActive]}>{value}</Text></Pressable>)}</View></View>)}<Pressable disabled={!displayName.trim() || isCreatingFan} onPress={() => void submit()} style={[styles.submit, (!displayName.trim() || isCreatingFan) && { opacity: 0.45 }]}>{isCreatingFan ? <ActivityIndicator color="#fff" /> : <><Feather name="star" size={18} color="#fff" /><Text style={styles.submitText}>FAN 소환하기</Text></>}</Pressable></ScrollView></NeonBackdrop>;
}
