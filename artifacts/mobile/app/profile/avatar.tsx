...
import RedesignedAvatarCustomizationScreen from "@/components/AvatarCustomizationScreen";

export default RedesignedAvatarCustomizationScreen;
...
function LegacyAvatarCustomizationScreen() {
  ...
  const slots: AvatarSlot[] =
    data?.appearance.type === "star"
      ? ["star_form", "effect"]
      : ["base", "head", "wear", "effect", "full_skin"];
  React.useEffect(() => {
    if (data?.appearance.type === "star") setSlot("star_form");
      ...
      >
        ...
        <View style={styles.stageBadge}>
          <Text style={styles.stageText}>
            {data.appearance.type.toUpperCase()} · Class{" "}
            {data.appearance.classStage}
          </Text>
        </View>
        <ScrollView
          horizontal
        ...
        >
          {slots.map((item) => (
            <Pressable
              ...
              style={[styles.tab, slot === item && styles.tabActive]}
            >
              <Text
                style={[styles.tabText, slot === item && styles.tabTextActive]}
              >
                {SLOT_LABEL[item]}
              </Text>
            </Pressable>
        ...
        {data.appearance.type === "star" ? (
          <Text style={styles.guide}>
            STAR 외형은 성장에 따라 자동으로 확정됩니다. Class 1은 Lv.10, Class
            2는 Lv.30, Class 3는 Lv.60에 열려요.
          </Text>
        ) : null}
          ...
          {items.map((item) => {
            ...
            return (
              <Pressable
                ...
                style={[
                  styles.card,
                  item.equipped && styles.cardActive,
                  locked && styles.cardLocked,
                ]}
              >
                <View style={styles.cardPreview}>
                  <CharacterAvatar
                    uri={previewRecipe(item)}
                    crop="full"
                    style={StyleSheet.absoluteFillObject}
                  />
                </View>
                ...
                <Text
                  style={[styles.cardMeta, item.equipped && styles.equipped]}
                >
                  {item.equipped
                    ? "장착 중"
                    : locked
                      ? `Lv.${CLASS_LEVEL[item.classStage] ?? "-"}`
                      : item.owned || item.isDefault
                        ? "보유"
...
const styles = StyleSheet.create({
  ...
  balanceValue: { color: "#B774FF", fontSize: 17, fontWeight: "900" },
  hero: {
    alignSelf: "center",
    width: 252,
    height: 360,
  ...
  stageText: { color: "#C595FF", fontSize: 12, fontWeight: "800" },
  ...
  tabActive: { backgroundColor: "#7132EF", borderColor: "#A55EFF" },
  tabText: { color: "#9E96AA", fontSize: 12, fontWeight: "700" },
  ...
  cardActive: { borderColor: "#A951FF", backgroundColor: "#170A27" },
  cardLocked: { opacity: 0.48 },
  cardPreview: { width: "100%", aspectRatio: 0.82, backgroundColor: "#05050A" },
  cardName: {
    color: "#F6F1FA",
    fontSize: 13,
    fontWeight: "800",
  ...
  equipped: { color: "#B86DFF", fontWeight: "800" },
  primary: {
