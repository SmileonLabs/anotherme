...
router.get("/admin/members/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({ member, profiles, sections: { fan: fan ?? null, mode: mode ?? null, star: stars, nft: stars.filter((star) => star.ownershipStatus === "verified"), wallets, activity: { postCount: Number(postCount?.total ?? 0), reportCount: Number(reportCount?.total ?? 0), blocks } } });
});
